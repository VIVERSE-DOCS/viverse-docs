...
Here is a code sample of a rendering pipeline implemented in WebGL that draws a single triangle to the screen. Toggle the "JS" and "HTML" tabs to view the usage of WebGL and the canvas API, respectively. Below, we describe how WebGL and the canvas interact with the GPU to draw the triangle onto the screen.
...
Animations make up a significant portion of scripting time in a complex scene, since they often trigger every frame. To reduce some of this overhead, developers can choose to render some animations every other frame, or at a fixed frame rate. This is particularly useful for background animations, like trees or grass moving in the wind.

:x: Full frame rate:

<pre class="language-typescript"><code class="lang-typescript"><strong>class StationaryAvatar {
</strong><strong>    public update(delta: number) {
</strong>        for (let i = 0; i &#x3C; this.instanceCount; i++) {
            const worldMatrix = this.instances[i].getWorldMatrix();
            lookAtToRef(worldMatrix, this.camera.getWorldMatrix(), this.tempMatrix);
            this.instances[i].updateWorldMatrixFrom(this.tempMatrix)
        }
    }
}
</code></pre>

:white\_check\_mark: Half frame rate:

<pre class="language-typescript"><code class="lang-typescript"><strong>class StationaryAvatar {
</strong><strong>    private shouldUpdate = false;
</strong><strong>
</strong><strong>    public update(delta: number) {
</strong><strong>        if (!this.shouldUpdate) {
</strong><strong>            this.shouldUpdate = true;
